    ...
    <div className='grid md:grid-cols-2 grid-cols-1 gap-4 text-white bg-blue-400'>
      {coreCharacters.map((i, n) => (
        <div className='flex flex-col justify-around m-5 p-5 bg-black font-inter md:tracking-wider '>
          ...
          <p key={n}className='mb-1'><span className='font-bold'>Name </span>: {i.alias}</p>
          <p key={n}className='mb-1'><span className='font-bold'>Name </span>: {i.abilities}</p>
          <p key={n}className='mb-1'><span className='font-bold'>Name </span>: {i.iconicQuotes}</p>
        </div>
