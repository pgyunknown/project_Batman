    ...
    <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-4 p-4 text-white">
      {movies.map((i, n)=>(
        <div key={n} className='bg-black  rounded-xl text-sm md:text-xl flex flex-col justify-around p-2 md:p-5 font-inter tracking-wide'>
          ...
          <h1 className='m-2 flex md:justify-center justify-around text-base md:text-2xl'>{i.title} </h1>
          <p className='m-1 md:m-2'><span>Year: </span>{i.year}</p>
          <p className='m-1 md:m-2'><span>Rating: </span> {i.rating} ⭐</p>
          <button className='text-sm md:text-xl'>watch trailer</button>
        </div>
      ))}
  </div>
  );
